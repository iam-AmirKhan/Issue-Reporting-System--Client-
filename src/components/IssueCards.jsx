...
export default function IssueCards({ issue, currentUser, onUpvoted }) {
  ...
  const isOwner =
    currentUser && (issue.createdBy === currentUser.id || issue.reporterId === currentUser.id);
  ...
  const handleUpvote = async () => {
    ...
    try {
      ...
      if (updated) {
        setUpvoteCount(
          updated.upvoteCount ?? (updated.upvoters ? updated.upvoters.length : upvoteCount)
        );
  ...
  const imageSrc =
    issue.image ||
    (issue.photos && issue.photos[0] && (issue.photos[0].url || issue.photos[0])) ||
    "/placeholder.png";

  return (
    <div
      className={`
        bg-white text-black rounded-lg shadow p-4 transition transform 
        hover:scale-[1.02] hover:shadow-xl 
        ${issue.boosted ? "ring-2 ring-yellow-300" : ""}
      `}
    >
      ...
      <div className="mt-3 flex justify-between items-start gap-2">
        <div>
          ...
          <p className="text-sm text-black">
            {issue.category || "uncategorized"} • {issue.location || "—"}
          </p>
        </div>

        <div className="flex flex-col items-end gap-2">
          <div className="text-xs px-2 py-1 rounded-full bg-slate-200 text-black">
            {issue.status || "open"}
          </div>

          <div
            className={`text-xs px-2 py-1 rounded-full ${
              issue.priority === "high" ? "bg-red-200" : "bg-slate-200"
            } text-black`}
          >
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`
              ...
              ${localUpvoted ? "bg-green-600 text-white" : "bg-slate-100 text-black"}
            `}
          >
            <svg
          ...
          {issue.boosted && (
            <span className="text-xs px-2 py-1 bg-yellow-100 rounded text-black">Boosted</span>
          )}
        </div>
